data_path = 'test.txt'

import pandas as pd
import numpy as np
import math
import matplotlib.pyplot as plt

df = pd.DataFrame(columns=['X', 'Y', 'W', 'Lidar_Data'])

f = open(data_path)
for line in f:
    readed_line = line
    curr_data = [readed_line[:-1].split(';')[0].split(','),
                 readed_line[:-1].split(';')[1].split(
                     ',')]  # [0] - odo, [1] - lidar
    print(type(curr_data[0]),len(curr_data), curr_data)
    print(curr_data[0][0])
    parsed_data = []
    for i, l in enumerate(curr_data):
        # print(l,i)
        if i == 0:
            for j, el in enumerate(l):
                parsed_data.append(float(el))
        else:
            parsed_data.append([float(item) for item in l])
            # print(len(l))
    # в итоге 4 ячейки: float, float, float, list
    # print(parsed_data)
    df.loc[len(df)] = parsed_data

# смотрим DataFrame
df.reset_index(inplace=True)  # доабвляем столбец индексво для подписе точек
print(df)
print(df[['X', 'Y', 'W']])
print(df[['Lidar_Data']])

# 681 точка лидара
# print(df.iloc[0]['Lidar_Data'])

plt.figure('Odometry1')
# plt.subplot(1, 2, 1)
plt.title('Path')
plt.xlabel('X')
plt.ylabel('Y')
plt.plot(df['X'], df['Y'], 'o')
ax1 = plt.gca()
df.apply(lambda x: ax1.annotate(x['index'], (x['X'] + 0.0, x['Y'])), axis=1)
plt.legend()
plt.grid()

# plt.subplot(1, 2, 2)
plt.figure('Odometry2')
plt.title('Angle')
plt.xlabel('Count')
plt.ylabel('W')
plt.plot(df['W'], 'o', color='g')
ax2 = plt.gca()
df.apply(lambda x: ax2.annotate(x['index'], (x['index'] + 0.0, x['W'])),
         axis=1)
plt.legend()
plt.grid()

rads = np.arange(0, (270 / 360 * 2 * np.pi), 270 / 360 * 2 * math.pi / 6.0001)
plt.figure('Lidar_0')
ax = plt.subplot(111, polar=True)
ax.plot(rads, df.iloc[0]['Lidar_Data'], 'r')


rads = np.arange(0, (270 / 360 * 2 * np.pi), 270 / 360 * 2 * math.pi / 6.0001)
plt.figure('Lidar_1')
ax = plt.subplot(111, polar=True)
ax.plot(rads, df.iloc[1]['Lidar_Data'], 'r')

rads = np.arange(0, (270 / 360 * 2 * np.pi), 270 / 360 * 2 * math.pi / 6.0001)
plt.figure('Lidar_2')
ax = plt.subplot(111, polar=True)
ax.plot(rads, df.iloc[2]['Lidar_Data'], 'r')

plt.figure('MAP')
plt.title('Map')
plt.xlabel('x')
plt.ylabel('y')

for i in range(3):
    delta_w = df.iloc[i]['W'] - df.iloc[0]['W']
    print()
    print(delta_w, i)
    rads_lidar = np.arange(0, (270 / 360 * 2 * np.pi),270 / 360 * 2 * math.pi / 6.0001)  # a
    print(rads_lidar)
    rads_real = rads_lidar + df.iloc[i][
        'W'] - 0.75 * math.pi #- 2*delta_w  # a-dw
    print(rads_real)
    x_dec_lidar = np.multiply(df.iloc[i]['Lidar_Data'], np.cos(rads_real))
    print('x_dec_lidar:',x_dec_lidar)
    y_dec_lidar = np.multiply(df.iloc[i]['Lidar_Data'], np.sin(rads_real))
    print('y_dec_lidar:',y_dec_lidar)
    x_map = df.iloc[i]['X'] + x_dec_lidar - 0.3 * math.sin(
        df.iloc[i]['W'] - 0.5 * math.pi)
    print('x_map', x_map)
    y_map = df.iloc[i]['Y'] + y_dec_lidar + 0.3*math.cos(df.iloc[i]['W'] - 0.5 * math.pi)
    print('y_map',y_map)

    false_obstacles_indexes = []
    for j, el in enumerate(df.iloc[i]['Lidar_Data']):
        if el >= 5.6:
            false_obstacles_indexes.append(j)

    # plt.plot(x_dec_lidar, y_dec_lidar, '.')
    # plt.plot(x_map, y_map, '.', color='r')
    x_map = np.delete(x_map, false_obstacles_indexes)
    print('x_map_clean',x_map)
    y_map = np.delete(y_map, false_obstacles_indexes)
    print('y_map_clean',y_map)
    plt.plot(x_map, y_map, 'o',
             color='r')  # карта по одним данным лидара в ориентации траектории робота
    plt.plot(df.iloc[i]['X'], df.iloc[i]['Y'], '.',
             color='k')  # положение робота

    pass
# print(df.iloc[3]['Lidar_Data'])
# print(df.iloc[4]['Lidar_Data'])
plt.show()
